 ...
 * Date: 13/01/2021
 * Time: 17:39
 * Project: Inlamningsuppgift1_Lambdas
 * Copyright: MIT
 */
...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsInDatabase(){
        ...
        return questions.stream().mapToInt(i -> 1).sum();
    }
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        ...
        return questions.stream().filter(q -> q.getCategory().equals(category)).mapToInt(i -> 1).sum();
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        ...
        return questions.stream().map(Question::getAllAnswers).flatMap(Collection::stream).anyMatch(a -> a.equalsIgnoreCase(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        ...
        return questions.stream().map(Question::getAllAnswers).flatMap(Collection::stream).filter(a -> a.equalsIgnoreCase(answerCandidate)).mapToInt(i -> 1).sum();
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c)
    {
        ...
        return questions.stream().filter(q -> q.getCategory().equals(c)).map(Question::getAllAnswers).flatMap(Collection::stream).reduce("", (answer1, answer2) -> answer1.length() > answer2.length() ? answer1 : answer2);
    }
